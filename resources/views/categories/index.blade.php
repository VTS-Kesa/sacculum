        ...
        <div class="mb-3 flex items-center gap-3 flex-wrap">
            <form method="POST" action="{{ route('categories.update', $category->slug) }}" class="flex items-center gap-3">
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        <input id="name" type="text" value="{{ $category->name }}" class="form-control @error('name') is-invalid @enderror" name="name" required autocomplete="name" autofocus />
            ...
            <form method="GET" action="{{ route('categories.delete', $category->slug) }}">
                @csrf
                <button type="submit" class="mt-3 bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded mb-3">Delete</button>
            </form>
        </div>
    @endforeach
